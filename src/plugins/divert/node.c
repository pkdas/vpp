...
static u8 *
format_divert_trace (u8 * s, va_list * args)
{
  CLIB_UNUSED (vlib_main_t * vm) = va_arg (*args, vlib_main_t *);
  CLIB_UNUSED (vlib_node_t * node) = va_arg (*args, vlib_node_t *);
  divert_trace_t *t = va_arg (*args, divert_trace_t *);

  s = format (s, "DIVERT: sw_if_index %d, next index %d\n",
...
extern vlib_node_registration_t divert_node;

#define foreach_divert_error \
_(SWAPPED, "Mac swap packets processed")

typedef enum
{
#define _(sym,str) DIVERT_ERROR_##sym,
  foreach_divert_error
#undef _
    DIVERT_N_ERROR,
} divert_error_t;

static char *divert_error_strings[] = {
...
VLIB_NODE_FN (divert_node) (vlib_main_t * vm, vlib_node_runtime_t * node,
			    vlib_frame_t * frame)
{
  u32 n_left_from, *from, *to_next;
  divert_next_t next_index;
  u32 pkts_swapped = 0;
  ...
  while (n_left_from > 0)
	...
	{
	  u32 next0 = DIVERT_NEXT_INTERFACE_OUTPUT;
	  u32 next1 = DIVERT_NEXT_INTERFACE_OUTPUT;
...
#undef _
	  ...
	  if (PREDICT_FALSE ((node->flags & VLIB_NODE_FLAG_TRACE)
	    ...
	    {
	      divert_trace_t *t = vlib_add_trace (vm, node, b0, sizeof (*t));
	      t->sw_if_index = sw_if_index0;
	      t->next_index = next0;
	      clib_memcpy_fast (t->new_src_mac, en0->src_address,
				sizeof (t->new_src_mac));
  ...
  vlib_node_increment_counter (vm, divert_node.index,
			       DIVERT_ERROR_SWAPPED, pkts_swapped);
			    ...
			    vlib_frame_t * frame)
{
  ...
  /*
   * We send all pkts to DIVERT_NEXT_INTERFACE_OUTPUT, aka
  ...
  while (n_left_from >= 4)
	...
	vnet_buffer (b[3])->sw_if_index[VLIB_RX];
      ...
      // next[0] = DIVERT_NEXT_INTERFACE_OUTPUT;
      // next[1] = DIVERT_NEXT_INTERFACE_OUTPUT;
      // next[2] = DIVERT_NEXT_INTERFACE_OUTPUT;
      // next[3] = DIVERT_NEXT_INTERFACE_OUTPUT;
      ...
      for (i = 0; i < frame->n_vectors; i++)
	{
	  if (b[0]->flags & VLIB_BUFFER_IS_TRACED)
		...
		vlib_add_trace (vm, node, b[0], sizeof (*t));
	      ...
	      t->next_index = DIVERT_NEXT_INTERFACE_OUTPUT;
			    ...
			    vlib_frame_t * frame)
{
  ...
  vlib_node_increment_counter (vm, divert_node.index,
			       DIVERT_ERROR_SWAPPED, frame->n_vectors);
  if (PREDICT_FALSE ((node->flags & VLIB_NODE_FLAG_TRACE)))
    {
      int i;
      b = bufs;
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (divert_node) =
{
  .name = "divert",
  .vector_size = sizeof (u32),
  .format_trace = format_divert_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,

  .n_errors = ARRAY_LEN(divert_error_strings),
  .error_strings = divert_error_strings,

  .n_next_nodes = DIVERT_N_NEXT,

  /* edit / add dispositions here */
  .next_nodes = {
    [DIVERT_NEXT_INTERFACE_OUTPUT] = "interface-output",
  },
};
/* *INDENT-ON* */
